...
app = typer.Typer()
_state = {"vault": None, "key": None, "storage": None}
...
def _unlock():
    ...
    if not storage.exists():
        ...
        master = pwm._safe_getpass("Choose a master password: ")
        confirm = pwm._safe_getpass("Confirm master password: ")
        if master != confirm:
            typer.echo("Passwords do not match.")
            raise typer.Exit(1)
        return pwm.create_new_vault(master, storage)

    master = pwm._safe_getpass("Enter your master password: ")
    try:
...
# ── vault commands (passwords) ────────────────────────────────────────


@app.command()
def view():
    """List all stored passwords."""
    _state["vault"], _state["key"] = _unlock()
    pwm.view_passwords(_state["vault"])
...
def add():
    """Add a new password."""
    _state["vault"], _state["key"] = _unlock()
    pwm.add_password(_state["vault"], _state["key"], _get_storage())
...
def modify():
    ...
    _state["vault"], _state["key"] = _unlock()
    pwm.modify_password(_state["vault"], _state["key"], _get_storage())
...
def delete():
    """Delete a password."""
    _state["vault"], _state["key"] = _unlock()
    pwm.delete_password(_state["vault"], _state["key"], _get_storage())
...
def change_master():
    ...
    _state["vault"], _state["key"] = _unlock()
    try:
        new_key = pwm.change_master_password(_state["vault"], _get_storage())
        _state["key"] = new_key
...
key_app = typer.Typer()
app.add_typer(key_app, name="key", help="Manage API keys and other secrets.")
...
def key_add():
    """Add a new API key."""
    _state["vault"], _state["key"] = _unlock()
    pwm.add_api_key(_state["vault"], _state["key"], _get_storage())
...
def key_list():
    ...
    _state["vault"], _state["key"] = _unlock()
    pwm.view_api_keys(_state["vault"])


@key_app.command(name="modify")
def key_modify():
    """Modify an existing API key."""
    _state["vault"], _state["key"] = _unlock()
    pwm.modify_api_key(_state["vault"], _state["key"], _get_storage())
...
def key_delete():
    """Delete an API key."""
    _state["vault"], _state["key"] = _unlock()
    pwm.delete_api_key(_state["vault"], _state["key"], _get_storage())
